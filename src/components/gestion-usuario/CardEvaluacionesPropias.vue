<template>
    <div class="w-fit h-fit bg-surface-900 flex rounded-md p-3 gap-4 justify-start items-center border border-surface-800 shadow-2xl ">
        <div class="overflow-hidden rounded-lg w-9/12">
            <Image :src="props.evaluacion.software?.portada" alt="Image" width="" preview class=" h-auto"/>
        </div>

        <article class="flex flex-col gap-2" >
            <div class="flex justify-start gap-4 "><!--Nombre, fecha-->
                <!-- <h4>{{ props.evaluacion.softwar }} </h4> -->
        ...
        <!--Botón acción-->
        <section class="w-fit">
            <Badge size="sm" :unstyled="true" :value="props.evaluacion.respuestas?.values" class="text-sm bg-primary py-2 px-4 rounded-lg text-surface-900 font-medium"/>
        </section>
    </div>
</template>
